...
In this challenge, I worked to identify the best inputs for a set of hidden mathematical functions without being able to see how the functions behaved internally. Instead, I was limited to a small number of trials and had to use the feedback from each one to guide my next decision. This reflects real-world problems where testing is expensive or time-consuming, such as tuning machine learning models, designing new materials or drugs, or improving engineering systems. The project therefore focuses on making efficient, well-judged decisions about when to try new ideas and when to build on promising results in order to achieve strong outcomes with very limited information.
